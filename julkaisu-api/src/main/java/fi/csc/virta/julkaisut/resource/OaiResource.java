...
import fi.csc.virta.julkaisut.oai.JulkaisuItemRepository;
    ...
    public Response handleOAI(@Context UriInfo uriInfo, @PathParam("context") String contextUrl) {
        try {
            ...
            final OAIDataProvider provider = new JulkaisuOAIDataProvider(xoaiManager, contextUrl,
                    new JulkaisuRepositoryConfiguration(uriInfo.getRequestUri().getPath(), config.getOaiConfig()), new JulkaisuSetRepository(julkaisuDAO),
                    new JulkaisuItemRepository(julkaisuDAO, metaDataPrefix));
